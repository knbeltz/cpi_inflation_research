...
def full_analysis_cpi(year): 
  def merge_data():
    ...
    merged_df = pd.merge(merged_df, df_debt_gdp[['Country', f'{year} debt_gdp']], on='Country', how='left')
  ...
  def train_test_X(): 
    ...
    X = merged_df_clean[[f'CPI {year}', f'Broad money (% of GDP) {year}', f'GDP growth (annual %) {year}', f'{year} debt_gdp', f'{year} unemployment rate %']]
